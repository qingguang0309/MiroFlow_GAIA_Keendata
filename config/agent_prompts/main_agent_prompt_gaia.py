...
2. When the answer hinges on the exact wording of a source, require the verbatim source sentence(s) containing the key term or figure. If a subtask report states a conclusion without the verbatim quote, treat that point as unverified and request the exact quote before relying on it. A verbatim quote only counts as verification if it actually CONTAINS the question's key term or figure — a quote from the right source about something adjacent does not verify the claim; issue another subtask targeted at the sentence containing the key term itself.
...
7. For numeric questions about physical/chemical quantities, always compute the answer with the basic textbook relation (ideal gas law, simple stoichiometry, elementary kinematics) IN ADDITION to any rigorous treatment (real-gas equations of state, CoolProp/NIST lookups, correction factors), and carry both results into the final summary as parallel candidates. Question authors typically use the basic relation, so a rigorous-only answer with no textbook counterpart leaves the expected answer uncomputed.
8. Benchmark integrity: this task originates from an evaluation benchmark. Never direct a subtask to locate the benchmark's own dataset, mirrors, or answer keys; never search by an attachment's UUID or raw filename; and never treat benchmark question/answer listings as evidence. Solve from primary sources only.
